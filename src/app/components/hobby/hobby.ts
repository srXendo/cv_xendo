...
import { HobbyInterface, HobbyPublicInterface } from './hobby.interface';
...
export class Hobby {
  hobby: HobbyInterface = {
    ...
    arr_hobby: [
      `Soy una persona curiosa, constante y apasionada por la <strong>tecnología</strong>. Desde muy joven sentí una fuerte atracción por entender cómo funcionan las cosas, lo que me llevó a adentrarme en el mundo de la <strong>programación</strong> y la <strong>ingeniería inversa</strong>. Disfruto enormemente de los desafíos técnicos, especialmente aquellos que implican desentrañar sistemas complejos o crear soluciones desde cero. He desarrollado diversos <strong>proyectos personales</strong> que reflejan esta inquietud, como <a href="https://torrenteservidor.es" target="_blank">https://torrenteservidor.es</a>, codigo fuente de <a href="https://github.com/weewoo-meow/torrente_online_web" target="_blank">torrenteservidor.es</a>, prueba de concepto de <a href="https://github.com/srXendo/torrente_online_matchmaker_directplay" target="_blank">matchmaker torrente online</a>. una iniciativa que me permitió recrear un servidor de matchmaking ya fuera de línea (Imatchmaker de Torrente), poniendo en práctica conocimientos avanzados de redes, protocolos y estructuras internas. También gestiono el repositorio <strong>srXendo</strong> en <a href="https://github.com/srXendo" target="_blank">GitHub</a>, donde comparto código, experimentos y herramientas útiles. He diseñado un <strong>dashboard empresarial</strong> que muestra datos clave para la toma de decisiones y una aplicación de gestión para el control de <strong>inventario, compras y ventas</strong> en tiendas. Me encantan los <strong>diagramas de flujo</strong> y estructurar la lógica de forma clara y ordenada, lo que me permite enfrentar problemas complejos con soluciones eficientes y bien pensadas.`,
  ...
  public_hobby: HobbyPublicInterface
  constructor(private sanitizer: DomSanitizer){
    this.public_hobby = this.get_safe_data(this.hobby)
  }
  ...
  get_safe_data(hobby: HobbyInterface): HobbyPublicInterface{
    const public_hobby: HobbyPublicInterface = {
      title_hobby: this.clean_data(hobby.title_hobby),
      arr_hobby: hobby.arr_hobby.map(row=>this.clean_data(row))
    }
    return public_hobby
  }
